    ...
    if (band == nBand) and (nBand > 1):
      ...
      try:
        wnRange = [refDict['band_lims_wvn'][0, 0],
                   refDict['band_lims_wvn'][1, nBand-1]]
      except:
        wnRange = [refDict['band_lims_wvn'][0, 0],
...
def statPDF(ref, test, outDir='.', prefix='stats_lblrtm_rrtmgp', \
  ...
  def diffCalc(dictRef, dictTest, bandNum, numCol, inVars, \
    ...
    if broadband:
      ...
      # we don't trust the broadband fields, so we'll use the by-band
      # fluxes and integrate ourselves
      for pVar in inVars:
        dictTest[pVar] = dictTest[pVar].sum(axis=2)
        dictRef[pVar] = dictRef[pVar].sum(axis=2)
      # end pVar loop
...
def forcingDiff(inRefBase, inTestBase, inRefForce, inTestForce, \
  ...
  # crashes if mdKey is not present, and sometimes it's not
  """
  mdKey = 'model-description'
...
if __name__ == '__main__':
  ...
  if conFile:
    ...
    # forcing is only being done if provided by config file now, 
    # no CLI arguments
    forcing = False
    ...
    if os.path.exists(rForceFile) and os.path.exists(tForceFile):
      forcing = True
      xt = cRefForceName
    ...
    sys.exit('PLEASE PROVIDE A CONFIGURATION (.ini) FILE')
